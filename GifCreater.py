...
CUSTOM_PARAMS = {
    ...
    "gif_fps": 15,
    "gif_loop": 0,
    "transparent_background": False,  # 保存截图时启用透明背景
    "remove_white_background": False,  # 额外消除白色背景像素
    "white_threshold": 5,  # 白色判断阈值，数值越大越宽松
    "Field_name": 'alpha.water'  # 颜色映射字段名称
}
# =========================================
...
for idx, (source_name, source) in enumerate(sources.items()):
    ...
    display.ColorArrayName = ['POINTS', CUSTOM_PARAMS["Field_name"]]
    alphaLUT = GetColorTransferFunction(CUSTOM_PARAMS["Field_name"])
